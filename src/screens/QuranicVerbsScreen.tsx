...
import {Alert, FlatList, Modal, SafeAreaView, TextInput, View} from "react-native";
...
export default function QuranicVerbsScreen(props: any) {
    ...
    const reloadVerbs = async function () {
        const fileName = `${FileSystem.documentDirectory}quranic-verbs.json`;
        FileSystem.downloadAsync(`${Constant.fileBaseUrl}quranic-verbs.json`,
            fileName,
        ).then((r) => {
            loadVerbs();
        });
